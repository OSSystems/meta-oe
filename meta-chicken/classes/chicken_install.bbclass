...
EGG = "${@bb.data.getVar('PN', d, 1).replace('chicken-egg-', '').replace('-native', '')}"
EGG_VERSION = "${@base_ifelse(bb.data.getVar('PV', d, 1) == "trunk", "", ":" + bb.data.getVar('PV', d, 1))}"
EGGDIR="${@bb.data.getVar('PN', d, 1).replace('-native', '')}-${PV}"
...
do_install () {
    CSC_OPTIONS="-L${STAGING_DIR_TARGET}${libdir} \
                 ...
                 ${TARGET_PREFIX}chicken-install -target -debug -prefix ${D}${localstatedir}
}
...
FILES_${PN} = "${localstatedir}/lib/chicken/*/*.*"
